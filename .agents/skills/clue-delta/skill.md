...
5. **PR + merge**: the PR is the review gate (human verifies meaning), the merge commit is the acceptance — of the change and of the `inferred` decisions it introduces alike. Merging promotes: the agent flips those decisions to `verified` with `accepted-by:` naming the merger, date and PR, in the next digest (or a final pre-merge commit when the merge was explicitly instructed). An explicit review objection to a specific decision keeps it `inferred` through the merge and becomes an open question. Run `clue-verify` first. `main` must never contain `/changes/`.
