...
4. **Digest** (final commits before PR): update permanent `/docs` (including README index blocks), apply any repo-local digest conventions the repo's AGENTS.md declares (e.g. recording user-visible impact in a changelog), write ADRs for decisions made, update plan bookkeeping, then **delete `/changes/CH-xxx-slug/`**. Digest precondition: every task in `tasks.md` is `[x]` or `[-]`-with-reason — an open `[ ]` means the change is not done, so finish it or downgrade it honestly.
