...
Before review, digest a full change into the permanent corpus, update its plan bookkeeping, and remove its `/changes/` workspace. Add a [`CHANGELOG.md`](CHANGELOG.md) entry when the change alters what an adopter receives — `clue`'s behaviour, a generated skill's text, or an artifact `clue init` or `clue scaffold` materializes into an adopter repository. A change confined to this repository's own corpus, contributor guidance, local conventions, or CI adds no release note even when it is a full Cliewen change, and neither does a plain editorial change ([C-002](docs/constraints/C-002-changelog-per-user-visible-change.md)). The final tree proposed for merge must not contain transient change files.
