...
- **Pull requests open ready for review, never as drafts.** The change loop now keeps unfinished work on its branch, runs the verification checklist, and only then opens the PR as the human review gate; `clue-delta` and `clue-verify` carry the rule.
