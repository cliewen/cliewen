...
- **Deciding how much process a change needs is now a three-rule test.** The skills, the `AGENTS.md` template, and the guide state change routing the same way: plain when nothing about meaning changes, light when meaning is touched but not changed, full for everything else — plus two guards, that an unclear tier takes the higher one and that discovering a decision or meaning change mid-work escalates immediately. The tier boundaries are unchanged, so work that was light or full before is light or full now.
- **Quality scenarios are now constraints.** The separate `quality` artifact type and its `docs/quality/` folder are gone; a verifiable quality bar (a coverage floor, a maximum onboarding time) is now recorded as a constraint with a `source` and an `enforcement` class, alongside every other cross-cutting rule. `clue init` no longer scaffolds a `quality/` folder, and brownfield extraction maps coverage and quality gates to constraints. An existing corpus that still has a `docs/quality/` folder keeps validating — the removal is from Cliewen's own taxonomy, not from the types a corpus may contain.
- **`clue validate` now accepts your own artifact types.** A type the validator does not recognize is treated as an adopter extension and checked against the default status lifecycle (`draft` → `active` → `retired`) instead of being rejected, so you can add your own kinds of record under `docs/` without changing the tool. All other checks — required fields, unique IDs, resolvable links, valid status — still apply. The status vocabulary is now one default lifecycle plus a short list of types that differ for a stated reason (goals, plans, decisions, the log, and transient workspace files), rather than a separate vocabulary per type.
- **Cliewen now states its core explicitly.** The methodology defines its three load-bearing elements — the verifiable thread, the human merge boundary, and `clue validate` as deterministic judge — behind a red line: changes that alter their meaning always require an explicit decision record and human acceptance. Everything else is periphery that adopters may freely extend with their own artifacts under `docs/`.
- **Cliewen now documents safe ongoing operation.** The guide explains exactly what the released tool and validator support, how to upgrade the binary, skills, and CI together, and how to recover from drift, skipped initialization files, validation failures, rollback, or removal without bypassing review. Every guide page now ends with one clear next action.
