...
- **`clue` installs with one command.** `brew install cliewen/tap/clue` on macOS and Linux, or `winget install --exact --id Cliewen.Clue` on Windows, replaces reading a platform table, downloading a binary, checking its SHA-256 by hand, renaming it, making it executable and editing your `PATH`. Upgrades become `brew upgrade clue` or `winget upgrade Cliewen.Clue`. The manual download is still documented for machines with neither package manager, and it is unchanged — including the assets it names. If you already run Cliewen in a repository, note that upgrading the binary this way does not touch the skills committed in that repository, so `clue validate` will correctly report drift until you update them too; the guide's *Operate safely* page explains how. (Windows availability follows shortly after this release: the winget submission is reviewed by Microsoft on its own schedule.)
