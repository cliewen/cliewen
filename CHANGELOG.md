...
- **Your agent now asks whether you are behind, when it starts.** `clue latest` only helps someone who already knows it exists, and the upgrade skill only runs when someone asks to upgrade — which is the request nobody makes when nothing has told them there is anything to upgrade to. The `AGENTS.md` that `clue init` materializes now opens by asking the agent to run `clue latest --quiet` once at the start of a session and to route a non-empty answer to `clue-upgrade`. A behind repository gets one line; a current one, an offline one, and one that simply cannot tell all get nothing. We put it in the hub rather than in a session hook on purpose: `AGENTS.md` is the file every assistant reads, and Cliewen ships no configuration for any of them — what your tools run is your business, not a methodology's. If you would rather have it run deterministically, wire the same command into your assistant's own session-start mechanism; it is built to be safe there.
