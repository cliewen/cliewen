...
- **Extraction no longer treats a JVM test's last AC tag as its chosen evidence.** A multi-tagged executable is now a rehearsal finding: the migration records its location, identities, assertions, and proposed resolution before source tests change. After human direction it either becomes separate one-criterion tests, or carries one reviewed primary criterion while every other identity gets a recorded dedicated-proof or `@draft` disposition. A normalizer may report the ambiguity but never silently delete a tag or choose the primary one.
