...
- **Analysis findings now say what their evidence can actually prove.** `clue-analysis` distinguishes a clean disposable environment from a prepared machine with local prerequisites, so a passing prepared build is not presented as onboarding reproducibility. Percentage and statistical claims now declare their population, eligibility and exclusions, sampling, uncertainty, and quality boundary instead of looking more certain than their evidence permits.
