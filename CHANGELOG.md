...
- **`clue validate` counts index rows that state their record but not what it is about.** The companion to the filler-row count, reported on the OK line and named by `clue validate --index-rows`, never failed on — every such row is one the tool wrote before it seeded descriptions. The two counts do not overlap: a row restating only its own link belongs to the first. Nothing clears either count for you, because regeneration preserves any row whose target still exists, so the number falls as you edit rows by hand. Cliewen's own corpus reports 80 of them at the time of writing.
