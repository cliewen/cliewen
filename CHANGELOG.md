...
- **The Cliewen validation workflow now tells you on every run whether GitHub actually blocks merging on the branch it checks.** The `validate` job adds a notice when the branch blocks deletion and force pushes, requires pull requests merged only by merge commit with conversations resolved, and requires the `validate` check, and a warning naming each missing rule when it does not. If GitHub cannot be asked, the warning says the state is unknown. The report never changes the job's result. The workflow's token cannot read classic branch protection or a ruleset's bypass list, so the notice says the bypass list went unseen, the warning says classic branch protection went unseen, and both are left for you to check. You get this by updating your caller's workflow reference.
...
- **No repository migration is required for this patch release.** `clue migrate` recognizes this release's generated skill files as a managed baseline; there is nothing else to apply.
...
- **An analysis can say where its findings ended up, so a spent spike can leave.** A spike is a measurement pinned to the revision it observed, not permanent truth, but until now nothing said when one had done its work — so findings accumulated in every corpus forever. An analysis may now carry `carried-by: [ID, …]` naming the durable artifacts its findings reached: an architecture or design overview, a decision, a capability. Once every plan that analysis serves is complete, `clue migrate` reports it as `MIG-013` with the evidence behind the claim. Both conditions are required, and the declaration is the half no tool can infer — a completed campaign says the campaign ended, not that its findings survived anywhere, so plan completion alone would sweep up every spike you have at once.
  ...
  **A spike that a live decision or constraint still cites is never reported, whatever its plans and carriers say.** A decision record is compact on purpose: it states what was decided, and the material that makes the reasoning checkable lives in the spike it came out of. Delete that and the rule is still readable but no longer reviewable. In a corpus where decisions routinely cite the spikes they came from, this leaves most of your analysis in place, which is the intended result — the population that can leave without cost is the transient register. A `links` entry is how a Cliewen record cites, so where a decision's link to a spike records provenance rather than evidence, remove the citation from the live decision in a reviewed change and the spike is reported on the next preview. That keeps pruning possible and keeps it an explicit human act.
...
- **A generated index row's status badge now follows the artifact it names.** `clue scaffold` built a row only for an artifact your index did not already reference, and kept an existing row exactly as it stood. That is right for the sentence describing the artifact — you write that, and regeneration must never overwrite it — but the badge beside it was a copy of a frontmatter field made once and never checked again. Change an artifact's status and its row kept the old value indefinitely; nothing reported the disagreement, and running the regenerator changed nothing, which is what made a stale row convincing rather than merely wrong.

  Regeneration now refreshes that badge and leaves the rest of the row alone. Three cases are deliberately untouched: a row you left without a badge gains none, a row linking several artifacts is left alone because no single artifact owns its badge, and a row whose artifact has unreadable frontmatter keeps what it had. A constraint's badge still shows its enforcement rather than its status, so a corpus with enforcement badges is not rewritten into nonsense. **Expect one larger-than-usual regeneration the first time you run `clue scaffold` after upgrading** — that is the accumulated drift being repaired, not new churn.
...
- **Constraint indexes now show the enforcement class that readers need.** `clue scaffold` and `clue init` append a constraint's `enforcement:` value in its index badge instead of the generic artifact status. `clue validate --index-rows` reports direct constraint rows whose existing badge disagrees with that field without failing or rewriting your curated index.
...
- **A push now claims nothing, and marking the pull request ready is the only readiness claim.** Until now the skills treated opening a PR as asserting a reviewed candidate, so unfinished work — including finished-looking review repairs — waited in a local worktree, where one was silently lost before a merge. Now every working session that changed anything ends by pushing the change branch, the pull request opens as a draft at the proposal commit (a light change at its first commit), and "ready for review" is the explicit act that binds green verification and a clean agentic review pass to the exact hosted head; a later substantive edit returns the PR to draft until its new head earns the same binding. On a draft, the CI digest gate runs without `--forbid-changes` and no acceptance brief is required — both bind when the PR is marked ready and on `main`, and a draft cannot be merged, so the merge gate is unchanged. Because publication starts with the first commit, the pre-publication rebase window is gone: a newer accepted `main` is always incorporated by merging it into the branch. The spec-first pause no longer asks whether to push; the proposal is already visible on the draft.
