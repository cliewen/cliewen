...
- **`clue validate` now catches hidden and duplicated frontmatter.** A UTF-8 byte-order mark anywhere in a corpus markdown file fails validation with guidance to strip it — a BOM hides the frontmatter fence from the parser. An artifact whose body opens with another frontmatter fence fails as a leftover second frontmatter, the shape a brownfield conversion leaves behind when it prepends instead of replaces; the `clue-extract` contract now spells out that a converted file carries exactly one frontmatter block.
