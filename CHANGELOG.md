...
- **`clue validate` now enforces six conventions that used to rely on an agent remembering them.** Hard-wrapped prose fails, naming the line the paragraph continues on. A `[-]` task with nothing after its checkbox fails, because an unexplained skip is indistinguishable from an abandoned one. A `proposal.md` that names no plan item and never says `plan-less` fails. An image link, or an image file stored under `docs/`, fails — a diagram in the corpus is inline Mermaid. A decision without `author` and `accepted-by`, or a capability without `goal`, fails; an empty `accepted-by` list counts as present, because an unsigned decision declaring itself unsigned is the record. And a plan table declaring a `Status` column has its cells read against `todo | doing | done | dropped`; a table with no status column is not a milestone table and is not read. If your corpus already followed these, upgrading changes nothing; if it did not, each finding names the file and the line you open to.
