...
When the user asks what is next, run `clue next --all`, report the first actionable milestone and the alternatives, then read the selected plan with `clue context` and apply **Before committing** below before asking whether to start. For a full change with a known identity, run `clue context <id>` and read its bounded slice. Otherwise read [`docs/README.md`](docs/README.md), choose the closest artifact, then run `clue context`; `/docs` is the system of record and working memory.
...
## Before committing: challenge the riskiest assumption
...
A commitment is consequential when being wrong would cost more than finding out first: adopting or revising a plan's promise, starting a milestone whose approach is unsettled, choosing between credible courses in a decision, or anything that reaches adopters. Simple work, work carrying out a course already challenged and understood, and work undoable within the same change proceed without a challenge and without a note explaining its absence. No prototype is universally required. This limit is part of the rule: a challenge demanded of everything is written by habit and challenges nothing ([PDR-061](docs/decisions/PDR-061-challenge-consequential-commitments-proportionally.md)).
