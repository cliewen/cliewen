...
`clue` is a single binary with no runtime dependencies. The primary route is a stamped release binary: open the [latest release](https://github.com/cliewen/cliewen/releases/latest), download `SHA256SUMS` and the asset for your operating system and architecture, verify the checksum, place it on `PATH` as `clue` (`clue.exe` on Windows), then run:

```sh
clue version
```

The [installation guide](https://cliewen.github.io/cliewen/getting-started#_1-install-clue) gives copyable checksum and `PATH` steps for Windows, macOS, and Linux. Release assets cover amd64 and arm64.
