...
**3. [Arm the wall](https://cliewen.dev/ci-wall).** The generated caller invokes Cliewen's upstream reusable validation workflow at an immutable reference and exposes only runner and binary-source choices. Only Markdown outside protected corpus, policy, configuration, and methodology paths is eligible to pass through without Cliewen; every non-Markdown, protected, mixed, empty, or unreadable diff fails closed to the corpus wall. Until you vendor the pinned release binary or choose the verified release source, those Cliewen changes pass with a visible warning. Once armed, the upstream workflow runs `clue validate --forbid-changes`; configure the protected branch for merge commits only, then use the guide's probe to prove both the history boundary and the failing pull-request block.
