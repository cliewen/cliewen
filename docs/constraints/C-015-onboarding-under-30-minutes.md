...
**Residual:** all of it. No test can time a human's first run, because the clock spans reading and installing as well as running; a reviewer judges whether a change to the quickstart or `clue init` keeps the journey under the bar. The mechanical path is held by AC-002, AC-024, and AC-025, and those prove the steps work — not that a person got through them in half an hour.
