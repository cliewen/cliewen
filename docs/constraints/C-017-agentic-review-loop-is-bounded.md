...
Three passes are the ordinary budget. A fourth or later pass runs only when the immediately preceding pass returned at least one blocking finding. The verification handoff reports the number of passes run together with the review mode, reviewed commit, and advisory findings left open.
