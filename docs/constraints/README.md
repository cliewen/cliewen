...
- [C-011 — Decision records are routed by type: ADR, PDR, or log row](C-011-decision-records-typed.md) · `human` — Cheap local choices enter the log; durable architecture and project/process choices receive their subject-specific record type.
...
- [C-015 — A new user reaches their first green validate in under 30 minutes](C-015-onboarding-under-30-minutes.md) · `human` — The onboarding path is judged by a timed clean-machine trial from discovery through successful corpus validation.
