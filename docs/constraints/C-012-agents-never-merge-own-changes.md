...
Before publishing a Cliewen change, the agent automatically reviews the committed candidate under [PDR-012](../decisions/PDR-012-agentic-review-before-publication.md), preferring a context-isolated read-only reviewer and disclosing an in-context fallback. Actionable findings return to the implementing context; each substantive fix is committed, checked against that commit, and reviewed again on the same commit, and local readiness requires a clean pass on the current commit. Before reporting any change ready, the agent commits every intended edit, runs the applicable local verification against that commit, requires a clean worktree, pushes the reviewed and verified commit, and confirms that the ready hosted PR's head branch and SHA equal the locally reviewed branch and `HEAD`. Review fixes to an unaccepted change invalidate the previous clean pass and repeat commit, local verification against the repaired commit, and agentic review of that same commit before push and hosted-head confirmation on the change's existing branch and PR. A human-requested local stopping point is preserved work but is explicitly incomplete and not mergeable. Building on an unmerged change is a blocking open question for the human ([PDR-007](../decisions/PDR-007-review-boundary.md)).
