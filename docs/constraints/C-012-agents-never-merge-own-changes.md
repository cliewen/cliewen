...
Before reporting any change ready, the agent commits every intended edit, runs the applicable local verification against that commit, requires a clean worktree, pushes the verified commit, and confirms that the ready hosted PR's head branch and SHA equal the locally verified branch and `HEAD`. Review fixes to an unaccepted change are committed and locally verified on that change's existing branch, the clean verified commit is pushed to its existing PR, and the hosted-head check repeats before the agent reports it ready again. A human-requested local stopping point is preserved work but is explicitly incomplete and not mergeable. Building on an unmerged change is a blocking open question for the human ([PDR-007](../decisions/PDR-007-review-boundary.md)).
