...
**Checked by:** index generation, which emits the stated form (AC-073), seeds the description (AC-096), keeps a row to one shape (AC-097), and preserves a curated description across regeneration (AC-098); and `clue validate`, which counts rows whose label restates only their own link and rows that state their record but say nothing about it, naming both behind `--index-rows` (AC-074, AC-099). Both counts are reported and never failed on, for the reasons in [ADR-041](../decisions/ADR-041-index-rows-state-their-record.md) and [ADR-046](../decisions/ADR-046-index-rows-say-what-the-artifact-is-about.md); under [C-004](C-004-never-weaken-checks.md) neither is softened to make the number look better.
