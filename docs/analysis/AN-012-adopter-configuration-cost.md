...
provenance: verified
reversal-cost: low
links: [P-008, M-035, ADR-013, ADR-011, ADR-022, ADR-030, CAP-004]
...
**Candidate 2 (a mechanical corpus upgrade path) is resolved as a capability, not yet as an event.** [ADR-039](../decisions/ADR-039-versioned-corpus-migrations.md) shipped `clue migrate` under P-009/M-041, and the adopter's own recorded activity shows the *specific* 69-issue gap this analysis measured closing by ordinary means before the tool would have been needed: CH-006 (PR [#223](https://github.com/robocode-dev/tank-royale/pull/223), part of the accepted-unit range [AN-010](AN-010-adopter-change-overhead.md) now covers) added the required `reversal-cost` field to 67 existing artifacts and corrected the one bad `status` value, in a full-tier change, months before `clue migrate` existed. Running the current source build's `clue validate` against the adopter's current corpus finds **zero** of the original 69 issues — no `reversal-cost`, `provenance`, `status`, or `ADR-035` hits at all.

**What replaced them is a different, larger population this campaign did not anticipate.** The same command reports **536 blocking issues** against the current corpus: 474 are [C-001](../constraints/C-001-no-hard-wrapped-markdown.md) (hard-wrapped Markdown — the constraint itself was registered 2026-07-14 by CH-011, well before this analysis's 2026-07-23 pin; what is newer is its machine enforcement, the hard-wrap check CH-110 added 2026-08-04), 58 are C-009 (a capability README's required `goal` field — the constraint likewise predates the pin, and CH-110 tightened its required-field enforcement in that same change), 2 are ADR-045's residual-declaration rule, and 2 are a newer constraint-source-resolution check pointing at a source file the adopter's corpus does not have. The finding is not that Cliewen added new rules since the pin; it is that Cliewen promoted rules that already existed to blocking machine checks — a materially different upgrade-cost story for the adopter than "new obligations arrived." The adopter is pinned at `clue-version: 0.11.2` against Cliewen's current release `0.14.1` — three minor releases behind, narrower than the four this analysis originally measured, but not closed, and `clue migrate` (candidate 2's shipped remedy) has not been run: its existence lowers the cost of closing this gap, it does not by itself close it.
