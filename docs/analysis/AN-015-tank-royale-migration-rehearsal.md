...
provenance: verified
reversal-cost: low
links: [P-009, CAP-003, PDR-004, PDR-020, PDR-021, ADR-037, AN-013]
title: A retrospective rehearsal on Tank Royale proves report-only migration truth and leaves one CI conjunct open
---

# AN-015 — A retrospective rehearsal on Tank Royale proves report-only migration truth and leaves one CI conjunct open
...
This is the durable record of the P-009/M-042 report-only rehearsal produced under CH-093. The rehearsal changed no file in the Tank Royale checkout, no target tests or routing, and no hosted state; this document is the one durable Cliewen artifact it produced, landed under explicit human direction (2026-08-01) rather than under PDR-020's mutate phase, which CH-093 did not authorize.

## Pins and evidence boundary

M-042 requires a human-selected repository: Robocode Tank Royale was named by the human initiating the change as the representative target, and no agent chose it.
...
The current source tree has already been adopted, so the **source pin** is `4e579878fd6667fab3b75515b6e68135a935c8df`, the first parent of adoption merge `b7fb320ccec1f4742ef923cb315e7dd84f7e824f`; this is the last source state containing `openspec/` before the target's CH-001 removed it.
...
The target's recent history contains merge commits for its CH-001 through CH-006, including the adoption merge, so local history is compatible with Cliewen's supported merge-commit provenance shape. Hosted branch protection and its exact merge-mode enforcement were not read from local evidence and remain unverified here.
...
Approval passes through the human merge boundary under PDR-004, and PDR-021 requires a merge commit for a full change: the target's local history for its CH-001 through CH-006 shows merge commits, so its observed practice matches the supported shape.
...
Replacing the copied CI wall with the upstream thin caller is high reversal cost and low confidence from here: the change is semantic, adopter-owned, and cannot be previewed mechanically, which is why `clue migrate` refuses it; the human has directed that replacement as a Tank Royale change (see the recorded answers below).
...
The rehearsal itself had no reversal cost. It wrote nothing to the target, which is what let every judgment above stay revisable until a human ruled on the open questions.
...
No target mutation was authorized by the rehearsal, so these mappings and deletions remain proposed historical evidence rather than a new extraction.
...
**The proposed upstream thin-caller wall was not run in the target's CI shape.** The target still runs the copied wall, replacing it is a semantic adopter-owned change the report-only milestone may not make, and a caller cannot be exercised in the target's CI without writing to the target. What the rehearsal can say is that the target's observed CI shape — GitHub-hosted Ubuntu, a writable `/usr/local/bin`, no self-hosted or no-root constraint in evidence — is inside the shape the upstream unit already supports; whether the caller actually runs there stays unproven until the adopter makes that change.

Three further limits belong to this target rather than to M-042's exit list, and are named so the milestone's met conjuncts are not read as stronger than they are. M-042 asks the report to inventory these and to record the target's own results separately; it does not ask the rehearsal to exercise them.

**The target's own test result is a configuration failure, not a suite verdict.** The separated-results conjunct is met — the target's runner result and Cliewen's deterministic results are recorded apart — but the target half is a `:bot-api:typescript:npmTest` task-configuration defect under a toolchain the target's documents describe inconsistently, so nothing here says the product suite is green or red.

**Stable-ID preservation was inventoried but not exercised.** The source pin declared no criterion identities at all, so ADR-037's multi-segment and letter-suffixed grammar surviving a migration is untested here. A source corpus that carries stable IDs is still needed before Cliewen can claim preservation works on real brownfield input; a reproducible fixture corpus is the named route (see the doors below).

**The pre-mutation checkpoint itself was not exercised.** The rehearsal reconstructs an extraction that already happened rather than previewing a pending one, so PDR-020's ordering guarantee — report, then human authorization, then mutation — is described from history and not demonstrated. M-042 requires that the rehearsal be produced with no target mutation, which held; it does not require that the ordering guarantee be demonstrated on an unconverted repository.

The rehearsal therefore supports a narrower boundary: Cliewen can describe and validate an already-adopted corpus without mutating the target, but it cannot claim that this target's current CI, supported build environment, or managed-carrier upgrade path is ready, that stable source identities survive migration, or that the pre-mutation checkpoint holds on an unconverted repository.
...
The rehearsal raised four blocking questions in its change workspace; the human answered all four on 2026-08-01. The two that direct Cliewen work are recorded in this corpus; the two that direct Tank Royale work belong to that repository and are carried here only as named doors.

**The copied CI wall (was OQ-001):** the human directs its replacement with the upstream thin-caller contract as a Tank Royale pull request. That adopter change, once merged and green, closes the one unmet M-042 conjunct with real CI evidence rather than a recorded boundary.

**The target test result (was OQ-002):** the human, as Tank Royale maintainer, names JDK 17+ (`CONTRIBUTING.md`, `DEVELOPMENT.md`) as the authoritative statement; the architecture report's 17–21 range is the document to correct. The `npmPack` task-dependency defect is Tank Royale work and blocks any target suite verdict until fixed.

**Durable evidence (was OQ-003):** the human directs that the rehearsal report survive the merge as this analysis artifact, landed under explicit human direction rather than under PDR-020's mutate phase.

**Phase closure (was OQ-004):** the human accepts that the migration-readiness phase closes once the Tank Royale wall replacement lands, without a second rehearsal against an unconverted repository. Exercising stable-ID preservation and the pre-mutation checkpoint on unconverted input is routed to a reproducible fixture corpus as a future plan door, following the precedent of the merge-mode and upgrade-path fixtures.

## Rejected alternatives

Running `clue init` or `clue migrate --apply` against Tank Royale was rejected because the milestone is report-only and the copied caller already produces a blocking semantic finding.
...
Hunting for a second, unconverted rehearsal target carrying stable source IDs was rejected by the human: no candidate is in reach, and the open mechanism questions — extended-ID preservation and the pre-mutation ordering — are answered better by a deterministic fixture than by an indefinite search.
...
The target needs the human-directed adopter change that replaces the copied validation wall with the upstream thin-caller contract; that change is also what closes M-042's last conjunct.

A reproducible fixture corpus carrying multi-segment and letter-suffixed source IDs remains a future plan door for exercising ADR-037 preservation and PDR-020's pre-mutation ordering on unconverted input.

P-009 M-043 and M-044 remain in the plan as of this landing; the rehearsal does not widen either contract. The human has additionally directed (2026-08-01) that M-044 — named but locally unproven foreign evidence — be executed, because Tank Royale tests already prove Cliewen claims with no honest way to record it, and that M-043 — durable dependent-change authorization — be removed with a recorded boundary rather than built, because the one-change-in-flight rule keeps stacked unaccepted changes out of supported practice. That plan revision is its own change with its own decision record.
