...
- Use an inspection ladder for third-party programs: black-box behavior first, intentionally bundled source where its terms permit inspection, and decompilation only to diagnose a concrete compatibility failure after the applicable permission or legal basis and information boundary are established.
...
The initial comparison overstated the boundary twice: first by treating reverse engineering as categorically prohibited without author permission, then by replacing that claim with conclusions from one jurisdiction. The robots are distributed by authors around the world, and copyright limitations and exceptions vary by country; [WIPO describes that national variation](https://www.wipo.int/en/web/copyright/limitations/index) rather than defining one global software-inspection rule. Cliewen must not turn a jurisdiction-specific exception into methodology.

A coding agent has no independent permission merely because it is an agent. Its inspection stays within the authority granted to the person or organization operating it and the rules applicable to the particular robot, activity, and place; using an agent is not a jurisdiction-selection mechanism.

Some Robocode archives intentionally include their source code. Reading that bundled source is source inspection rather than reconstructing source from bytecode and is the preferred white-box debugging path when the archive's license, terms, and applicable rules permit the intended use. Availability of source does not by itself settle permission to copy, modify, redistribute, or publish the bot's protected expression or algorithm.

Black-box runtime evidence remains the preferred default and can distinguish many bridge failures from robot incompatibilities by running a lawfully obtained robot in environments the tester is permitted to use, supplying controlled Robot API inputs, and comparing observable calls, events, outputs, errors, timing, and scores. This avoids source reconstruction but does not create a universal legal safe harbor; the applicable license, terms, and law still govern the activity.

Per-robot metadata should record the robot and version, origin, declared license or terms, test environment, observed failure, suspected category, evidence, eligibility status, explicit exclusion reason, inspection mode (`black-box`, `bundled-source`, or `decompiled`), intended use of any derived information, and the permission or case-specific legal basis for source-level inspection. If that basis is unclear, the trial stops at permitted black-box evidence or intentionally bundled source whose intended use is established; it does not assemble an ad hoc survey of national law. When decompilation is permitted, its sole project purpose is debugging a specific bridge compatibility failure, and examination is limited to the code relevant to that failure. It is not undertaken to discover, reproduce, reuse, compare, or publish a robot author's strategy; algorithm details encountered incidentally are not used or disclosed.

Manual decompilation is out of scope: it is too time-consuming to be a practical debugging path. If decompilation is permitted and necessary, a coding agent performs it and the human receives only the minimum bridge-facing diagnosis, never reconstructed source, source excerpts, or strategy details. Agent-only processing reduces direct human exposure, but the trial must not promise that no human can ever see the decompiled code or that no system retains it unless those properties are verified for the execution environment. The handling record should name the agent surface and provider, training and retention controls, local or hosted transcripts and logs, administrator or provider access, artifact lifetime, and deletion procedure. Decompiled artifacts stay out of source control and durable findings. An unverified provider claim is not reassurance offered to robot authors.

Eligibility categories might include obsolete Java dependencies, reflection or runtime tricks incompatible with modern Java, unsupported team behavior, and failures demonstrably present outside the bridge. Findings should expose bridge-facing evidence rather than algorithm details. The categories and inspection ladder are proposed analysis output, not adopted policy. No legacy robot binary was decompiled during this calibration.
