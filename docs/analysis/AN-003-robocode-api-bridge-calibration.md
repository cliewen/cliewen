...
Agent-only processing can reduce direct human exposure, but the trial must not promise that no human can ever see the decompiled code or that no system retains it unless those properties are verified for the execution environment. The handling record should name the agent surface and provider, training and retention controls, local or hosted transcripts and logs, administrator or provider access, artifact lifetime, and deletion procedure. Decompiled artifacts stay out of source control and durable findings; agent output contains only the minimum bridge-facing diagnosis, never source excerpts or strategy details. An unverified provider claim is not reassurance offered to robot authors.
