...
This record is CH-147's inspection of the populations named by `go run ./cmd/clue validate --read-cost` at its proposal head. It records the judgment that command deliberately cannot make: whether a rendered-document split has distinct consumers, and whether an over-budget identity has a genuinely redundant outgoing relationship. It does not turn either population into a pass/fail target or a registry; later corpus changes rerun the command and receive a new population.

## Multi-document artifact

| Artifact | Outcome | Reason |
| --- | --- | --- |
| AN-022 | Accepted | M-064's reader needs the non-prose scoring to interpret the Pattern C determination that follows it; they are one investigation with shared evidence, so splitting them would fragment that reader's conclusion. |
...
| AN-013 | Accepted | Its campaigns and milestone, core evidence and contract decisions, capability owners, constraints, and related analyses preserve all three distributed-work gaps. |
...
| CAP-003 | Accepted | Its goal, extraction and identity decisions, parity and imported-work architecture, and rehearsal rule define the brownfield-extraction capability. |
