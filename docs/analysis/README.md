...
- [AN-009 — Multi-agent review repairs can remain outside the pull request](AN-009-multi-agent-pr-handoff.md) · `active`
...
- [AN-013 — The corpus cannot say what is accepted, where its evidence actually lives, or which repository a reference means](AN-013-distributed-work-and-evidence-boundaries.md) — the three deferred distributed-work boundaries reproduced: a dependent change whose merge binds unaccepted meaning, cross-repository evidence discharged by hand and inexpressible, and 50 unqualified forge references of which one already resolves to the wrong pull request
- [AN-014 — A confidential migration assessment exposes evidence, identity, and enterprise-operation gaps](AN-014-confidential-migration-assessment.md) — the anonymous supplied verdict is bounded as unreproduced target evidence while its JVM cross-product, ID grammar, merge-mode, CI, and upgrade claims are separated into locally reproduced mechanics and explicit hypotheses consumed by P-009
- [AN-015 — A retrospective rehearsal on Tank Royale proves report-only migration truth and leaves one CI conjunct open](AN-015-tank-royale-migration-rehearsal.md) — the two-pin rehearsal record, the one unmet M-042 conjunct, the target's limits, and the four recorded human answers of 2026-08-01
- [AN-016 — Disposable fixtures prove the composed brownfield migration contract](AN-016-disposable-brownfield-migration-fixtures.md) · `active` — CH-120's numeric and opaque fixture evidence for report-only rehearsal, authorized mutation, target validation, parity, carrier reconciliation, and source-work survival
- [AN-017 — The brownfield migration assessment re-derived as an inspectable gate register](AN-017-sanitized-migration-assessment-gate-register.md) · `active` — This is a sanitized record of the supplied brownfield migration assessment that originated P-011.
<!-- clue:index:end -->
