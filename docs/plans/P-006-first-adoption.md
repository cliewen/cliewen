...
| M-021 | **`clue init` accepts a real repository's skills folder**: initialization into a repository whose agent-skills directory is a symlink, or already holds skills Cliewen does not own, neither writes through the link nor overwrites foreign skills, and says what it skipped; the behavior is covered by an acceptance criterion with a positive and a negative test; `clue validate` and `go test ./...` green | `todo` | |
