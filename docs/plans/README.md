...
- [P-015 — Cliewen's read cost stays flat as a corpus grows](P-015-corpus-read-cost.md) · `active` — Bounds what a context slice prints and names what the bound holds back, makes read cost and multi-document artifacts measurable rather than asserted, then repairs what those measurements find and only that.
- [P-016 — Decision records are concise, typed, and migration-safe](P-016-concise-decision-records.md) · `active` — Retires the decision log in favour of subject-typed decision records, keeps future records to enduring decisions, migrates older adopter repositories without silently misclassifying their log rows, and compacts the existing decisions in bounded review batches.
<!-- clue:index:end -->
