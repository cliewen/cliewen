...
- [P-014 — Cliewen gives a surviving superseded record a machine-visible edge](P-014-supersession-edge.md) · `active` — Opened with M-069 as its sole milestone, the door PDR-038 named: decide, with a fuller argument than the first pass, whether `supersedes:` widens to cover a decision record that is superseded but survives, or whether that residue is declined again.
...
- [P-016 — Decision records are concise, typed, and migration-safe](P-016-concise-decision-records.md) · `active` — This campaign replaces the decision log with subject-typed ADRs, PDRs, and IDRs; keeps future records focused on enduring decisions; migrates older adopter repositories without silently…
<!-- clue:index:end -->
