...
- [P-012 — Cliewen closes the brownfield migration gap on re-derived evidence](P-012-migration-gap-closes-on-evidence.md) · `completed` (2026-08-07) — Finished the re-derived migration gates: accountable deferrals, derived reports, an explicit registry limit, and assessment-scale composed evidence.
- [P-013 — Cliewen is simplified against a stated criterion](P-013-simplification.md) · `draft` — Deferred by every campaign that named it; holds PDR-013's "does the core need it?" applied to a measured surface, the accumulate-only corpus, and the distributed-work findings P-011 closed without touching.
<!-- clue:index:end -->
