...
| M-005 | **Greenfield in one command**: `clue init` emits the taxonomy, AGENTS.md routing, skills and CI template; CAP-001 (onboarding) goes `active` with its criteria tested, the 30-minute install→green promise held as quality scenario QS-002 *(exit criterion revised by CH-020, a declared plan revision: AC-001 split per the granularity rule — decision-log row 2026-07-17)* | `done` | CH-020: `clue init` (embedded templates, ADR-018), README quickstart, CAP-001 `active` — mechanical path tested as AC-002/003/024/025, the 30-min promise recast as QS-002 (AC-001 retired) |
