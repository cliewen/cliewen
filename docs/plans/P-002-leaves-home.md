...
| M-005 | **Greenfield in one command**: `clue init` emits the taxonomy, AGENTS.md routing, skills and CI template; CAP-001 (onboarding) goes `active` with its install→first-green-validate-in-30-min criteria tested | `todo` | CH-020 delivered `clue init` (embedded templates, ADR-018), the README quickstart, and CAP-001 `active` (AC-002/003/024/025 tested; AC-001 retired to QS-002 per the decision-log row 2026-07-17). The criterion's 30-minute wording no longer matches the AC split — revising it is a semantic plan change awaiting its own PR; M-005 stays open until that lands. |
