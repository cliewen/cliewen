...
| M-005 | **Greenfield in one command**: `clue init` emits the taxonomy, AGENTS.md routing, skills and CI template; CAP-001 (onboarding) goes `active` with its install→first-green-validate-in-30-min criteria tested | `done` | CH-020: `clue init` (embedded templates, ADR-018), README quickstart, CAP-001 `active` — mechanical path tested as AC-002/003/024/025, the 30-min promise recast as QS-002 (AC-001 retired) |
