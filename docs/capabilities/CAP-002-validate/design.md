...
3. **AC↔test contract** (`corpus.checkACTests`, CH-003): `@AC-xxx` tags harvested from `criteria.md` bodies; test references harvested per [ADR-005](../../decisions/ADR-005-test-reference-convention.md) — framework-native tags are the general convention, and Go's `testing` has none, so the Go harvester reads function names (`Test…AC<digits>…`) from `*_test.go`, scanning the code tree while skipping `docs/`, `changes/` and hidden directories. Both directions checked: an AC in **active** criteria without a test fails (draft criteria are exempt — that is what draft means), and a test referencing an undeclared AC fails.
