...
1. **Command (seconds):** `clue init` materializes the whole convention — `/docs` taxonomy, marked architecture and design overview bootstraps, README indexes, skills, CI workflow — in one call. The agent replaces those two markers with concise repository truth before the first green validation. `clue migrate` later upgrades a released corpus through a previewed, safe plan and creates a missing overview bootstrap without replacing local prose. A legacy decision log is the semantic exception: MIG-010 inventories every row and blocks every write until a reviewed full change classifies future-shaping choices by subject, accounts for narrative, repairs references, and removes the log.
...
The emitted CI caller delegates its validation wall to Cliewen's immutable upstream reusable workflow while keeping runner and binary-acquisition choices local to the adopter ([ADR-038](../../decisions/ADR-038-upstream-validation-workflow.md)). Init writes the branch-protection checklist that makes that wall block merges beside the caller, because the host setting is the one part of the wall no file in the repository can carry ([PDR-060](../../decisions/PDR-060-the-validation-workflow-reports-the-merge-boundary.md)).
