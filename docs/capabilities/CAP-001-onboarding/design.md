...
- **Release binaries are the primary install path** (learned 2026-07-22): Windows, macOS, and Linux instructions select the published architecture asset, verify it against `SHA256SUMS`, put it on the user `PATH`, and show the version result. Source installation is secondary, so using Cliewen does not appear to require Go.
