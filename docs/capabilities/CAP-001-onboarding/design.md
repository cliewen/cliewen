...
- **Release binaries are the primary install path** (learned 2026-07-22; the installer-script clause superseded 2026-07-27 by [ADR-030](../../decisions/ADR-030-verified-install-scripts.md)): a compact platform table identifies the published architecture asset, and short manual steps verify it against `SHA256SUMS`, put it on the user `PATH`, and show the version result. The current unsigned macOS path names the checksum-first Gatekeeper exception explicitly. Source installation is secondary, so using Cliewen does not appear to require Go.
