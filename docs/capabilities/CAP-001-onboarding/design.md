...
- **Operations must name the support boundary and recovery path** (learned 2026-07-23): the public guide distinguishes released binaries, the Go and Java/Kotlin test-reference harvesters, generated skill locations, the GitHub workflow, and repository-local validation from broader methodology intent. It gives a coordinated binary/skills/CI upgrade path, keeps `init`'s skip behavior explicit, and treats extraction, adoption, and uninstallation rollback as reviewed repository work rather than a bypass. Every guide page ends in one primary next action, checked by AC-036's positive and negative tests.
