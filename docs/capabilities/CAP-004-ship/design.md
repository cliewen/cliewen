...
Every release publishes **both** asset shapes under one `SHA256SUMS`: the bare binaries under the exact names the hand-rolled loop used (`clue-<version>-<os>-<arch>`, `.exe` on Windows), and `tar.gz`/`zip` archives a later package-manager channel can consume. That checksum file lists upload names, so an adopter's vendored wall — which downloads one bare binary by name and runs `sha256sum -c --ignore-missing SHA256SUMS` — passes unchanged with the extra archive lines present. The bare names are a public contract with two dependents, not an implementation detail ([ADR-030](../../decisions/ADR-030-verified-install-scripts.md)): the scaffolded wall and the install scripts under `guide/public/`. `TestSanity_ReleaseKeepsTheAssetNamesTheAdopterWallInstalls` and `TestSanity_InstallScriptsUseTheReleaseAssetContract` tie the emitted names to both, so they cannot drift apart silently.
...
- **The install scripts are not exercised by CI** — their contract with the asset names is tested, and both were run end to end against a real release before merge, but no job installs `clue` on three operating systems on every push. The operational proof stays the first user on a platform.
