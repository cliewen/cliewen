...
That the two versions in fact agree is checked, not assumed. Ahead of the tests and the build, the workflow runs `clue validate --forbid-changes` from source with the tag stamped in (`go run -ldflags "-X main.version=${VERSION}" ./cmd/clue`), so `checkSkillVersions` judges the release candidate exactly as an adopter's installed binary will judge their checkout. A tag that disagrees with the shipped skill stamp fails there, naming both versions, instead of publishing a self-inconsistent pair whose only symptom is unexplained drift in an adopter's repository. Reusing the shipped rule keeps one definition of the invariant — no second, shell-resident copy that can drift from `checkSkillVersions` — and `TestSanity_ReleaseRunsTheJudgeStampedAsTheTag` holds the step, its stamp, and its position ahead of the build and the publish step.
