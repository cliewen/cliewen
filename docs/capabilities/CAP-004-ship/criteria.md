...
Feature: clue ships — a versioned binary and versioned skills, drift made lintable
  ...
  @AC-147
  Scenario: Every generated lifecycle skill carries the same decision-record contract
    Test-type: Unit
    Given the canonical decision-record instruction and both distributed generated skill trees
    When a maintainer runs the skill generator
    Then every lifecycle skill receives the same ADR, PDR, and IDR subject-routing and compact-shape instruction
    And both distributed trees match their canonical generated bytes
    But no generated skill routes a decision by reversal cost or into a legacy decision log
```
