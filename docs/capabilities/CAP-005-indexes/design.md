...
- **An appended row says what the artifact is about** ([ADR-046](../../decisions/ADR-046-index-rows-say-what-the-artifact-is-about.md)): the row gains ` — <description>`, seeded by `corpus.RowDescription` from the target's own body — a lede paragraph beneath the H1 where one exists, otherwise the first sentence of the first paragraph under the first heading, skipping headings, tables, lists, blockquotes, and fenced blocks, bounded at a word boundary so the row stays one line (AC-096). Every link in the sentence is reduced to its label, a code span included, because the block's link reading knows nothing about spans and a quoted placeholder target would leave generation emitting a corpus the judge rejects. An artifact with no readable sentence keeps the shorter row rather than carrying an empty tail (AC-097). The seed is a first draft: regeneration rewrites nothing that exists, nothing backfills a description, and a curated row survives — though not once its target is gone (AC-098). `clue validate` counts the rows still saying nothing (AC-099), disjoint from the filler count because a filler row carries no status badge.
