...
Agents may independently implement, review, and repair changes without losing work between sessions: every working turn that changed anything ends with the change branch pushed, the pull request exists as a draft from first publication so unfinished work stays visible, review results name the hosted commit they cover, actionable findings remain visible on the pull request, an authorized dependency on unmerged work remains explicit until digest, and marking the pull request ready is the explicit act that binds verification and a clean review to the exact hosted head.
