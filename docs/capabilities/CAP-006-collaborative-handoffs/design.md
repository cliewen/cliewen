...
- **Race handling:** if the remote moved from `S`, the updater fetches and reconciles without force, then reruns checks and review because the candidate changed. A merged or closed pull request is terminal; unpublished work waits for explicit human scope.
