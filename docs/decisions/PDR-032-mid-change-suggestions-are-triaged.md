...
**No carrier outside this record states this rule today.** The goals folder README carries only the [ADR-002](ADR-002-inbox-is-proposed-goals.md) inbox mechanism this decision reuses — that an idea enters the corpus as a goal with `status: proposed` — and not the triage that routes a suggestion to it. The carriers this rule is owed to — the `clue-delta` loop's Propose and Digest steps, the shared `durable-work` fragment, and the change-workspace template — do not yet state it, and that debt is [P-013](../plans/P-013-simplification.md)'s M-067 rather than an omission left unrecorded. `clue` will hold none of it in any case: a suggestion arriving in conversation is invisible to a judge that reads files, so the rule is agent-enforced and the constraint M-067 mints states its residual.
