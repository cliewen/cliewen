...
The goals folder README carries the [ADR-002](ADR-002-inbox-is-proposed-goals.md) inbox mechanism this decision reuses — that an idea enters the corpus as a goal with `status: proposed`. The triage rule itself is stated, since [P-013](../plans/P-013-simplification.md)'s M-067, by the shared `durable-work` fragment — rendered as a required local reference in every generated skill directory that carries a change loop, with `clue-delta`'s `skill.md` routing a reader there when a change starts or resumes or a suggestion arrives — and by the register entry [C-021](../constraints/C-021-mid-change-suggestions-are-triaged.md), `enforcement: human`, which states this rule's own residual: `clue` holds none of it, since a suggestion arriving in conversation is invisible to a judge that reads files.
