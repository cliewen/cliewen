...
The goals folder README carries the [ADR-002](ADR-002-inbox-is-proposed-goals.md) inbox mechanism this decision reuses — that an idea enters the corpus as a goal with `status: proposed`. The triage rule itself is stated, since [P-013](../plans/P-013-simplification.md)'s M-067, by the shared `durable-work` fragment — rendered into every generated skill that carries a change loop — which `clue-delta`'s own "apply Durable work state throughout the loop" sentence points a reader at from its Propose step onward, and by the register entry [C-021](../constraints/C-021-mid-change-suggestions-are-triaged.md), `enforcement: human`, which states this rule's own residual: `clue` holds none of it, since a suggestion arriving in conversation is invisible to a judge that reads files.
