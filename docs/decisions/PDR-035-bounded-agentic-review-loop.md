...
- **“No blocking findings” is the exit condition without weakening the exact-commit boundary.** This settles PDR-012's older “no actionable findings” wording in favour of the severity model already carried by `clue-verify`. An advisory repair may ride before a pass already required by a blocking repair. An advisory first reported by a pass with no blocking findings stays visible and open for a later change; editing the clean commit would create a new candidate that [PDR-016](PDR-016-pr-state-carries-agent-handoffs.md) and [C-012](../constraints/C-012-agents-never-merge-own-changes.md) require to be reviewed, while this decision says the bounded loop is over. The advisory is deferred, not silently repaired and not promoted into a gate.
...
**Carrier inventory:** [PDR-012](PDR-012-agentic-review-before-publication.md) carries the amended decision; [PDR-016](PDR-016-pr-state-carries-agent-handoffs.md) and [C-012](../constraints/C-012-agents-never-merge-own-changes.md) retain the exact-reviewed-commit rule this decision reconciles with the bounded loop; [C-017](../constraints/C-017-agentic-review-loop-is-bounded.md) registers the standing human-enforced rule; the canonical `internal/skills/source/skills/clue-verify.md.tmpl` states the operating procedure and generates the repository and scaffolded copies; the shared `internal/skills/source/shared/review-boundary.md.tmpl` states the advisory-deferral consequence in every skill that carries the exact-commit boundary; `internal/skills/generate_test.go` pins their stable clauses; `guide/change-loop.md` and `CONTRIBUTING.md` carry the same operating and handoff rules for their own reading paths; and the repository and scaffolded pull-request templates require the review mode, reviewed commit, pass count, and advisory findings left open, with the template assertion in `cmd/clue/main_test.go` holding that field. `clue-delta`, `clue-extract`, and the repository and scaffolded routing hubs only invoke or require the review loop outside the generated review-boundary fragment and do not state its severity or pass budget, so their other prose remains unchanged.
