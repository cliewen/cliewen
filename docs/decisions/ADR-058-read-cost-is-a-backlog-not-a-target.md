...
- **Residual: an accepted row is indistinguishable from an unexamined one.** ADR-057's report writes no registry and derives every row from current corpus state, so acceptance lives where a reader finds it — the artifact's own prose, or the plan whose milestone worked the backlog — and nothing joins that reason back to the row mechanically. A campaign that inspects the whole population therefore ends with the report unchanged, and the evidence that the work happened is the milestone's record rather than a smaller number. Closing this gap would mean the registry ADR-057 refused, which costs more than the ambiguity does.
