...
- **Notes are required before the tag, not after.** A raised stamp with no `## [X.Y.Z]` section fails here. The release workflow would fail too ([ADR-012](ADR-012-release-notes-from-changelog.md)), but a version number cannot be un-spent, so the cheaper failure is the one that happens before the tag exists. Failing at the merge turned out to be cheaper than the release workflow and still too expensive: tagging fires only on the merge that raises the stamp, so a release refused here cannot be retried and the number is abandoned. The gates therefore live in `.github/scripts/release-gates.sh`, which CI runs on the pull request as well, where the version is still recoverable. This workflow remains the authority — the pull-request run is an earlier copy of the same check, not a substitute for it.
...
**Carrier:** `.github/workflows/tag-on-merge.yml`, `.github/scripts/release-gates.sh` and `TestSanity_TagOnMergeDerivesTheVersionAndStartsTheRelease`, which holds the version source, the dispatch, the permission it needs, the already-tagged no-op, and the notes requirement in both the workflow and the pull-request caller.
