...
- **File and format:** `.clue/id-ledger.yaml` at the repository root — not under `docs/`, which holds authored corpus prose with per-file frontmatter, while the ledger is one generated/operational registry file. Every entry has the exact canonical `id`, `kind` (`numeric | opaque`), and `state` (`reserved | live | retired`). A numeric entry also has its `prefix` and decimal `component`; a per-prefix `counters` map holds the last-issued numeric component. An opaque imported entry has no numeric component or counter: its exact canonical `id` is preserved verbatim, and a mapping may add it only through that namespace's documented source-owned generator. Entries imported from a brownfield source also carry `source-revision` and `source-location` (M-052's original fields).
...
- **O(1) lookups:** both queries an agent needs — "is this ID used" and, for a numeric namespace, "what is the next ID for this prefix" — are in-memory map lookups after a single `Load()`, never a corpus scan. The `counters` map makes numeric allocation an increment; a `byID` map makes exact canonical-ID existence a lookup for both numeric and opaque entries. An opaque namespace's next ID remains the output of its documented source-owned generator, which is checked against `byID`. Scanning the corpus stays necessary only once, for backfill, and afterward only as a cross-check inside `clue validate`.
- **Validator cross-check:** `clue validate` rejects a live artifact whose exact canonical ID is `retired` or `reserved` under a different reservation in the ledger, and rejects a live artifact with no ledger entry once the ledger is the corpus's source of truth (gated so a corpus without a ledger yet is unaffected). It rejects a numeric entry without a valid decimal component and an opaque entry with one, and it rejects an opaque ID not preserved verbatim from its source mapping. This composes with, and does not replace, ADR-034's `supersedes:` mechanism: `supersedes:` gives a reader a pointer to what replaced a deleted ID; the ledger gives the allocator a reason never to reissue that ID's number in the first place.
