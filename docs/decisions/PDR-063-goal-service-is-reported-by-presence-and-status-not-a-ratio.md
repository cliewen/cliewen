...
Both threads matter: a capability is the intent thread's ongoing capacity, while a plan reaching `completed` is the delivery thread's existing signal that a one-time achievement finished. A goal such as "Cliewen is public" is served by a completed plan and no standing capability at all; reporting capabilities alone would have reported it as unserved.
