...
- **Source manifest** (YAML, one per source-mapping run, written during the `clue-extract` rehearsal and revised only when the source is re-read at a new revision): top-level `source-revision` and `source-location` pin what was read; each `entries[]` row names the criterion `id` and either:
  ...
  - a `disposition` of `draft | human | retired` with a `justification` naming the source location and, for `draft`, the named plan door PDR-024 already requires. An entry may not combine `excluded` with a proof class, nor a proof class with a `disposition` — each row states exactly one outcome; an exclusion or disposition is the sole row for its ID.
