...
Regeneration rewrites no description that already exists and never backfills one. **Amended by [ADR-064](ADR-064-regeneration-owns-the-index-badge.md):** the badge beside the description is generated content and is refreshed; this record's protection covers the sentence, which is the part an author writes. A dead target drops its row regardless of its description. `clue validate` reports rows that identify their record but say nothing about it as a visible population, not an `Issue`.
