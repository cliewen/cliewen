...
- **The sentence stays on one line.** A continuation line is not an entry `checkIndexes` recognizes, so extraction truncates at a sentence boundary rather than wrapping, backing off to a rune boundary where the prose carries no spaces to cut on so the row can never be invalid UTF-8.
- **A candidate that cannot be made safe is declined, not repaired.** Two shapes would corrupt the block they are written into: a link label carrying its own brackets, which defeats any single reduction pattern while the block's link reading matches the residue anyway, and an index marker or HTML comment, which truncates the block on the next run and strands prose outside the markers. Both yield the shorter row instead. Declining is the rule rather than a better pattern because the first attempt at this shipped exactly such a pattern and it was wrong.
- **Every link in the sentence is reduced to its label, a code span included.** A live link would make the row cover a second target, so a prose reference to another artifact would satisfy that artifact's index entry and let a real row for it go missing. A code span is no exception, even though its contents are literal text to a markdown reader: the block's link reading knows nothing about spans, and an artifact quoting the row format it defines writes a placeholder target that resolves to nothing. Preserving it would make index generation emit a corpus the judge then rejects, which is the outcome this decision and ADR-041 both refuse.
...
- **A description does not keep a dead row alive.** A row whose target no longer resolves is dropped however well curated, so deleting an artifact still deletes its entry ([ADR-034](ADR-034-retirement-is-deletion.md)). The same reading governs the extraction guidance, which absorbs a foreign index file into the rows and then deletes the file rather than leaving it inert beside them.
...
Failing here would also invert where the fault lies. Almost every description-less row in a Cliewen corpus is a row the generator wrote before this decision existed, and the rest are artifacts whose body holds no readable opening sentence, where the generator still emits the shorter row by design. A judge that rejected the tool's own prior output, in a file the adopter owns, would turn a Cliewen defect into the adopter's red build on upgrade.
...
**Carrier:** the appending branch and the extractor in `regenIndex` (`internal/scaffold/scaffold.go`), the description population in `corpus.IndexDescriptionBacklog` and its CLI surface (`internal/corpus/index.go`, `cmd/clue/main.go`), [C-016](../constraints/C-016-index-rows-state-their-record.md), the `clue-extract` absorption guidance, and CAP-002's and CAP-005's criteria and design.
