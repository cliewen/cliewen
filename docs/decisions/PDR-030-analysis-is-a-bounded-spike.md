...
The rules are also not separable. Remove "a spike is disposable" and "its findings are not" says nothing; remove "end in a findings document" and the disposability rule licenses losing the work; remove "feed it to a consumer" and the corpus accumulates analysis nobody asked for. A record per sentence would state a set of rules each of which depends on all the others.
...
One record per sentence, each stating something meaningless without the others. The register that found this gap lists the statements, and the tempting repair is one trace apiece. It would produce a corpus where reversing any one clause leaves the rest internally contradictory, and where the reason the rules hold together — that a bounded investigation, a disposable body, a durable finding, and a real consumer are one design — is stated nowhere.

## Rejected: promote the Foundation Document to a valid trace

It would close this gap and every one like it in a single edit, and it is wrong for the reason the document itself states: it is frozen, it is never edited, and where it and the corpus disagree the corpus wins. An artifact that cannot be amended cannot carry a live rule, because the first time the rule needs to change there is nowhere to change it.
