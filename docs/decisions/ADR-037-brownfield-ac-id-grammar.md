...
The malformed-identity diagnostic is scoped to declared namespaces. A tag or declaration token is judged against this grammar only when it already names a declared prefix, allowing for carrier underscores and case, so `SNAP_SQS_001B` and `snap-sqs-001` are diagnosed as wrong spellings of a declared identity. Every other token stays untouched: ADR-036 keeps tags outside every declared AC namespace as ordinary runner metadata, and an adopter's `@Tag("java-17")` or prose `@v1-2` is not a criterion reference merely because it contains a separator and a digit.
