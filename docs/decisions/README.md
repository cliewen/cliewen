...
- [ADR-047 — Diagram representation preserves links and assets](ADR-047-diagram-representation-preserves-links.md) · `inferred` — Choosing Mermaid, ASCII art, or SVG is human judgment rather than a validation verdict; image links and assets are valid corpus content, and extraction retargets or reports a link instead of dropping it.
- [PDR-024 — Extraction never silently discards acceptance evidence](PDR-024-extraction-never-silently-discards-evidence.md) · `inferred` — A normalizer may report a multi-identity or class-level JVM tag but never deletes, selects, or relocates it; the rehearsal inventories it and a human chooses the resolution.
- [PDR-025 — Truthful brownfield migration precedes simplification](PDR-025-brownfield-migration-precedes-simplification.md) · `inferred` · P-012-as-simplification clause superseded by PDR-026 — P-011 becomes the brownfield-migration campaign and simplification is deferred behind it, because an extraction that can look green while changing what the source proves is the higher-risk boundary.
- [ADR-048 — A persisted ledger replaces scan-and-max allocation for every native ID prefix](ADR-048-corpus-wide-id-ledger.md) · `inferred` — Widens M-052's ledger to cover all native ID prefixes, not only imported criteria, so a retired ID's number is never re-minted; allocation and existence checks become O(1) lookups against `.clue/id-ledger.yaml` instead of a corpus scan.
- [ADR-049 — Migration parity compares a pinned source manifest against a derived target manifest](ADR-049-migration-parity-manifests.md) · `inferred` — A source mapping pins its own manifest; `clue parity` derives the target from the corpus and ledger and reports every missing criterion, orphaned tag, changed evidence, stale fingerprint, or unjustified draft/Human/retirement disposition.
- [ADR-050 — In-flight source work becomes a durable imported-change record, never a transient workspace](ADR-050-imported-change-records.md) · `inferred` — A new `imported-change` artifact type holds a source change's pinned origin, intent, design rationale, dependencies, and task-to-criterion proof links; a `complete` record's proof links must all resolve to proven criteria, and the record is never deleted or retired.
- [ADR-051 — A pinned carrier inventory and a deterministic reconciliation check close the operational-carrier gap](ADR-051-carrier-reconciliation-manifests.md) · `inferred` — ADR-048 closes the identity gap and ADR-049 closes the evidence-parity gap for a brownfield migration, but neither reaches a third class of thing a source repository depends on: material that is not…
- [PDR-026 — A campaign closes on re-derived gate status, not on its own evidence column](PDR-026-campaigns-close-on-re-derived-evidence.md) · `inferred` — A campaign is closed by gate status re-derived from the corpus and the tool rather than by its own evidence column, a request it declined is recorded as declined rather than as satisfied, and simplification moves to P-013.
...
- [PDR-028 — A derived extraction report is not a committed per-criterion registry](PDR-028-derived-report-is-not-a-committed-registry.md) · `inferred` — The brownfield migration assessment asked for a committed per-criterion registry that a reviewer could read against a pull-request diff.
- [PDR-029 — Simplification is judged by two tests, chosen by surface](PDR-029-simplification-tests-by-surface.md) · `inferred` — PDR-013's *does the core need it?* passes almost everything on Cliewen's own methodology carriers, so carrier prose is judged instead by whether each statement traces to a corpus artifact, is stated once per reading path, and can be checked, with the most binding rules read first.
<!-- clue:index:end -->
