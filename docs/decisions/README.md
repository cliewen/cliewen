...
- [PDR-029 — Simplification is judged by two tests, chosen by surface](PDR-029-simplification-tests-by-surface.md) · `verified` — PDR-013's *does the core need it?* passes almost everything on Cliewen's own methodology carriers, so rule-bearing carrier statements are judged instead by whether each one traces to a corpus artifact, is stated once per reading path, and can be checked, with connective statements classified separately and the most binding rules read first.
<!-- clue:index:end -->
