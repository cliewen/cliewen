...
**Carrier:** the decisions folder README prose (ships as `clue init` template prose); the routing wording in the `clue-delta` and `clue-verify` skills (agent); the register entry [C-011](../constraints/C-011-decision-records-typed.md) that holds the routing rule. The rejected-record clause is carried by the same two READMEs, by the `clue-analysis` skill's canonical source, and, since [P-013](../plans/P-013-simplification.md)'s M-067, by the shared `decision-records` fragment rendered into all six generated skills — the carrier gap M-067 was owed to close.
