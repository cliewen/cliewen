...
|---|---|---|---|
| 2026-07-17 | The scaffolded constraints README carries the convention register; the register starts empty — no seeded C-xxx artifacts for methodology rules | The methodology's rules are carried and versioned by the shipped skills (the drift lint holds them); the register is for the adopter's own prose conventions, and seeding methodology copies would create a second, unversioned carrier. A template edit reverses this cheaply | CH-020 / PR #19 (review finding) |
