...
|---|---|---|---|
| 2026-07-18 | Change-loop PRs open ready for review, never as drafts; unfinished work stays on the branch until verification passes | The PR is the human review gate for a completed proposal, while draft status declares that the proposal is not ready for that gate; carriers are `clue-delta` step 5 and the final `clue-verify` check | CH-023 / PR #22 |
