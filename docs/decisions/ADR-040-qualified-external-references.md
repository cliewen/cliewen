...
type: decision
status: verified
links: [P-009, AN-013, ADR-005, ADR-009, ADR-039, C-013, CAP-002]
...
The judge does not depend on the convention being followed. A token counts as an attempted citation only when it carries a path or an identity complete with its digits, so an ordinary sentence using the word — and an index marker, and a capitalised word after the colon — keeps working even when someone forgets the backticks. The convention is what makes the corpus readable; the shape requirement is what keeps prose out of the diagnostic.

Fenced code, inline code spans, link targets, and heading anchors such as `#4-resume-game` are not references and are never read as one. A colour literal carrying a hex letter is excluded by the same rule that excludes an anchor. An all-digit colour such as `#777777` is indistinguishable from an issue number by shape alone, and it is read as a reference: excluding runs of that length would silence the numbers adopters actually cite, and a rule that cannot see the common case is not worth the rare false positive it avoids. A colour literal in prose belongs in a code span, which is ordinary markdown practice. The same holds for an ordinal: `the #1 cause of drift` is read as a citation for exactly the same reason, and belongs in a code span too — as this sentence demonstrates, since writing it bare made the rule reject its own explanation.
...
Only the half of a reference that resolves is resolved. An address inside a code span is a literal — a command example is being shown, not pointed at — and an address used as a link's readable name is prose, since this decision keeps link text free precisely so a familiar name survives. Both are read exactly as the judge reads them, from one shared rule rather than two that can drift: a span excluded when addresses are gathered but visible when they are rewritten is how `--apply` comes to edit text the run never classified.
...
**A credential sharpens the answer where it is honoured, and is sent nowhere else.** `GITHUB_TOKEN` or `GH_TOKEN` turns a private target from an oblique `404` into a plain answer — but only against the API host, because a bearer token is inert on the web host, whose pages authenticate by session cookie. The resolver therefore sends a credential only when the address has an API equivalent to ask instead, only after the unauthenticated answer was `gone`, and only to the service that issued it. The credentialed answer is read solely as "it exists" and never supplies a rewrite target — an API redirect names an API address, and writing that into a corpus would replace a readable link with an internal one, for whoever happened to hold a token. Attaching a forge token to an arbitrary wiki in the corpus would hand that host a credential it was never meant to see. Whoever holds a credential gets the sharper result; whoever does not still gets a report that never condemns a correct reference.
