...
- **ADR-011's version contract is narrowed to owned skills.** This decision supersedes [ADR-011](ADR-011-version-stamping.md)'s location-only enrollment of every `.agents/skills/*/skill.md`; ADR-011's per-skill version stamps, set consistency, binary drift failure, and `dev` exemption continue to govern the marked Cliewen set.
...
- **Legacy Cliewen names fail toward migration.** The six canonical directories (`clue-analysis`, `clue-delta`, `clue-extract`, `clue-plan`, `clue-upgrade`, and `clue-verify`) are reserved Cliewen slots. If one contains an unmarked `skill.md`, validation reports that the managed skill must be reinstalled. A new binary therefore cannot mistake a managed installation for a repository with no managed skills.
