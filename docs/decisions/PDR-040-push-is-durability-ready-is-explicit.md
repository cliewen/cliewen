...
type: decision
status: inferred
links: [G-001, PDR-007, PDR-012, PDR-016, PDR-033, C-004]
...
- **The relaxation is fail-closed and the caller declares it.** The `ready_for_review` trigger lives in the adopter-owned caller ([ADR-038](ADR-038-upstream-validation-workflow.md)), and without it no run fires when a draft becomes a candidate — the draft's lenient result would stand as the required check on a mergeable pull request. The reusable workflow therefore relaxes only for a caller that declares `draft-aware`, and keeps the strict gate otherwise, so bumping the workflow reference alone can never open that hole.
