...
*Only numbers cross the boundary.* What this command prints is a command the user is told to run, so no byte of the answer may reach the output unexamined. A tag is recognized only if it is exactly three decimal numbers, and the version that is printed is rebuilt from those numbers rather than echoed from the response. Anything else — a suffix, a newline, a shell fragment — is not a version this command knows, and an answer it does not recognize was already defined as *could not tell*. The same rule gates the cache, because a file any local process can write must be read as warily as the network.
