...
*Only numbers cross the boundary.* What this command prints is a command the user is told to run, so no byte of the answer may reach the output unexamined. A tag is recognized only if it is exactly three decimal numbers, and the version that is printed is rebuilt from those numbers rather than echoed from the response. Anything else — a suffix, a newline, a shell fragment — is not a version this command knows, and an answer it does not recognize was already defined as *could not tell*. The same rule gates the cache, because a file any local process can write must be read as warily as the network. The running stamp is the other side and is read the other way: it is local, it never reaches the printed recipe, and a release candidate is legitimately stamped `0.13.0-rc1`, so its suffix is trimmed and its numbers compared, with a pre-release ordered before the release it names — the day that release ships is exactly when its candidate's user needs to hear it. A stamp that cannot be read even then is reported as uncomparable — never as current, which would collapse the one distinction this command exists to make. A stamp *newer* than anything published is reported as unpublished for the same reason: there is nothing to catch up with, but the release list never named that version, and the command states only facts that list gave it. The rule that gates the network gates the corpus too: the drift message repeats a version string a skill file supplies, and the half of that message shaped as things to run is written only for a stamp that is three plain numbers.
