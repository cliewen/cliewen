...
- ***Does every statement trace, state its rule once, and admit checking?*** governs carrier prose: skills, the routing hub, contributor and public guidance, and CLI text. The unit is the individual statement, not the file, and it must pass all three conditions. It **traces** when it names, or is plainly derivable from, a live corpus artifact — a decision, a constraint, a goal, or an acceptance criterion. It **states its rule once** when no live carrier already states the same rule in the same reading path. It **admits checking** when a reader can determine whether it has been satisfied.

  Tracing is the operative condition, and it is deliberately mechanical rather than a judgment of merit. The question a human reviewer asks when reading a skill closely is *why do you have this statement?*, and the only answer that survives scrutiny is a corpus artifact the reader can open. "It seemed wise", "it has always been there", and a rationale that exists solely in the statement itself are not answers. A statement can be unique in the corpus and perfectly checkable and still fail, by being a true sentence tied to nothing.
...
Three carriers state this decision and move together: this record, P-013's milestones and prose, and the refinement blockquote in [PDR-013](PDR-013-explicit-core-red-line.md) that qualifies where *does the core need it?* reaches. That note is a live carrier in [PDR-019](PDR-019-methodology-contract-carriers-move-together.md)'s sense, not history — it states the carrier test in summary form inside a `verified` record that outranks this one at read time, so a change to the test that leaves it behind publishes a superseded rule from the more authoritative file. Any change to the conditions updates all three.
