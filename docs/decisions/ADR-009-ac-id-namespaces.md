...
- **The corpus is the registry.** Next-free-ID per prefix is `max + 1` over the declared set; a source corpus's parallel registry retires at extraction. [AN-002](../analysis/AN-002-model2diagram-extraction.md) preserves the concrete extraction evidence without making the private target's registry file a normative reference. Retirement tombstones (ADR-007) work unchanged in any namespace: `@MG-010 @retired`.
