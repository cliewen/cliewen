...
> **Scoped by [PDR-042](PDR-042-routing-recommends-contract-aware-effort.md):** mandatory agentic review applies to a chosen full loop. Simple work has no mandatory Cliewen review, though a repository or user may still request one.
...
**Every chosen full change receives an automatic adversarial agent review of its committed candidate before the ready mark, using a context-isolated reviewer when the coding-agent host supports it and a disclosed in-context fallback otherwise.**
