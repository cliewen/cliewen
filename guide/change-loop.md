...
`tasks.md` is an ordered checklist with dependencies first, and a task you mark `[-]` as infeasible says why on the same line. If a blocking question appears, write it to `open-questions.md` and stop. Record the answer as a typed decision only when it makes a future-shaping choice.

## 3. Implement

Before starting or resuming work for a milestone, the agent checks that the plan still serves its goal, the milestone is still wanted and achievable, and the remaining dependencies and order still make sense. It checks again when new evidence challenges the campaign. A passing check needs no record. If the plan no longer holds, the agent writes the mismatch and options in `open-questions.md`, pauses affected work, and asks a human to choose the replan. It declares that revision in the plan before resuming. Most replans are delivery adjustments, not decisions. A typed decision record is needed only when the selected course will constrain future work.
