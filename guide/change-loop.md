...
Create `ch-xxx-your-slug` from the current tip of `main`. One initiating author takes one initiated Cliewen change to its ready pull request before starting another, and a change never starts from unaccepted work. From the first commit onward, every working session that changed anything ends by pushing the branch: a push is durability, never a claim of readiness, and nothing waits in a local worktree where the next session cannot see it. Plain changes, reviews, and help updating an existing pull request do not consume another initiated-change slot. Work that genuinely depends on an unmerged change stops at a blocking question until a human authorizes it; the answered workspace record names the base, authorization, and meaning that a dependent merge would bind, and remains committed until digest.

## 2. Propose

A full change commits `/changes/CH-xxx-your-slug/proposal.md` before implementation, pushes it, and opens the pull request as a draft; a light change opens its draft at its first commit. The proposal says what will change, why it matters, which plan item it serves or that it is plan-less, and where the decision boundary lies. The draft is where unfinished work lives and stays visible: it claims nothing and cannot be merged.
