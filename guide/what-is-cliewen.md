...
Decisions inside a full loop follow the same rhythm. A decision an agent records is born `inferred`; merging the pull request makes it binding, and later explicit human approval promotes it to `verified`. A user who declines the full recommendation chooses simple process for that integration; the agent records the override risk in Git history rather than manufacturing a corpus decision log entry.
