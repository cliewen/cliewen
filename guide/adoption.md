...
Every extracted artifact starts inferred. Non-decision artifacts use `provenance: inferred` and `reversal-cost: low|high` to show whether deferring their meaning is safe. High-cost inferred meaning blocks an active capability in its immediate graph slice; low-cost meaning may remain deferred. Remove the field when human review promotes the artifact to `verified`. Decisions use `status: inferred` and `author: agent` because ADR/PDR routing already treats them as expensive.
