...
*What is next?* works as a prompt because `clue next` derives the repository's position, not because anything remembered your last session. It finds an open change to resume, unfinished plan milestones, or proposed goals when no campaign is active. `clue context <id>` gives the agent a small, relevant slice to read because the links between artifacts were written down while the reasoning was fresh. A decision made three months ago does not have to be re-derived, re-argued, or guessed at from a diff, because it is a record with its rejected alternatives still attached.
