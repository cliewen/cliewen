...
3. **Every Cliewen proposal declares which plan item it serves** (see [`docs/plans/`](docs/plans/README.md)) or explicitly declares itself plan-less. The merge digest updates plan bookkeeping in the same commit — including closing the plan: the change that completes the last milestone sets it `completed` in that digest, and designates a successor there when one is decided.
