...
Before loading the corpus, classify the requested work by its effect on meaning. A repository may define a narrow release route in its local conventions; that route remains a human-merged PR, but is neither plain work nor a Cliewen change when its declared release-only file surface is satisfied.
