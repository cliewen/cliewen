...
2. **Ready means the hosted PR contains the verified state.** Before reporting any change ready, commit every intended edit, run the applicable local verification against that commit, require a clean worktree, push the verified commit, and confirm that a ready hosted PR's head branch and SHA equal the locally verified branch and `HEAD`. Review fixes repeat that local-before-publish handoff on the existing branch and PR. A human-requested local stopping point is preserved work, but it is incomplete and not mergeable.
