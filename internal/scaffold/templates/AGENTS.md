...
2. **Ready means the hosted PR contains the reviewed and verified state.** Before publishing a Cliewen change, automatically run the `clue-verify` agentic review loop on the committed candidate: prefer a context-isolated read-only reviewer where the host supports it, otherwise disclose the in-context fallback; resolve actionable findings and repeat after every substantive fix until the current commit receives a clean pass. Then commit every intended edit, run the applicable local verification against that commit, require a clean worktree, push the reviewed commit, and confirm that a ready hosted PR's head branch and SHA equal the locally reviewed branch and `HEAD`. Review fixes repeat that local-before-publish handoff on the existing branch and PR. A human-requested local stopping point is preserved work, but it is incomplete and not mergeable.
