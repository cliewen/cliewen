...
4. Rejected alternatives that constitute a decision get a rejected decision record (ADR if architectural, PDR if project/process) in `/docs/decisions/`, not just a paragraph in the findings. The decision record is timeless: state the outcome and only the enduring context and rationale needed to understand it; the spike chronology, triggering incident, conversation, and change history stay in the findings, PR, and Git history.
