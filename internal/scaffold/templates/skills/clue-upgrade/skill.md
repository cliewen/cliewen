...
Classify scope before using the Cliewen loop. A repository may define a narrow release route in its local conventions: it remains an ordinary human-merged PR, but is neither plain work nor a Cliewen change when its declared release-only file surface is satisfied. Otherwise, three rules decide the tier, by how deeply the change reaches into meaning; take the first rule that matches.
