...
Plans in `draft` are proposed and do not supply actionable work; `active` plans supply the next work, while `completed` plans are history. `clue next [--all]` names an open change first, prioritizes `doing` before `todo` in active milestone tables, and reports draft milestones and proposed goals separately for human review.
