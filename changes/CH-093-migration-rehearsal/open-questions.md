...
**Answered (2026-08-01):** replace the wall in a Tank Royale pull request, made by the human as that repository's maintainer. That adopter change, once merged and green, closes M-042's one unmet conjunct with real CI evidence. Recorded in AN-015; the change itself is Tank Royale work and no Cliewen decision record is minted for it.
...
**Answered (2026-08-01):** JDK 17+ is authoritative; the architecture report's 17–21 range is the document to correct. The `npmPack` defect is Tank Royale work. Recorded in AN-015 as a carried door, not a Cliewen decision.
...
**Answered (2026-08-01):** yes — the human directs that the evidence be traceable. Landed as AN-015, `proposal.md` amended in the same change, decision recorded as a log row in the digest.
...
**Answered (2026-08-01):** the phase closes once the Tank Royale wall replacement lands — no second rehearsal target is hunted. Exercising stable-ID preservation and the pre-mutation checkpoint on unconverted input is routed to a reproducible fixture corpus as a future plan door. M-042 stays `todo` in this change and flips `done` when the wall PR is merged and green. The human additionally directed the P-009 tail: M-044 is executed as its own change, M-043 is removed with a recorded boundary — that plan revision is a separate change with its own decision record. Recorded in AN-015.
