---
id: CH-122-tasks
type: tasks
status: open
links: [CH-122]
title: Tasks for CH-122
---

# CH-122 — tasks

- [x] Re-derive the assessment's gaps and blocking gates from the current corpus and commands; record the proposed dispositions in `proposal.md`.
- [x] Write `proposal.md` and this task list.
- [x] Run the existing migration to seed this repository's identity ledger, then inspect the resulting allocation and removal of MIG-008 — ADR-052 makes an absent optional caller a notice; `go run ./cmd/clue migrate --apply` generated `.clue/id-ledger.yaml`, and `go run ./cmd/clue id next CH` allocated `CH-123`.
- [x] Write the sanitized assessment analysis and its gate-by-gate PDR-026 disposition register.
- [x] Record the branch-protection acceptance-evidence boundary and update its methodology carriers.
- [x] Repair CAP-003's stale adopter-CI binary-distribution claim against the reusable validation workflow and verified installation scripts.
- [x] Update M-057 bookkeeping, README indexes, and `[Unreleased]`.
- [x] Run focused tests and corpus validation.
- [x] Run `clue-verify` and resolve its review findings — in-context fallback reviewed `faa501c` after full build, vet, tests, coverage, corpus validation, and AC-124 scenario/test comparison; no blocking findings. The digest's deletion check runs against the resulting candidate.
- [ ] Push the reviewed commit and open a ready PR with its acceptance brief; confirm its hosted head equals `HEAD`.
